import os
from scripts import parse_all_weapon, parse_all_carryitem
...
def make_excel(output: str = 'output/faction'):
    ...
    xlsx_path = output + '/武器护甲数据汇总.xlsx'
        ...
        if item.is_dir():
            ...
            if item.name.endswith("武器"):
                for k in enum_type(item.path):
                    w_attrs = faction_all_weapon[sheet_name][k]
                ...
                for k in enum_type(item.path):
                    ci_attrs = faction_all_ci[sheet_name][k]
...
def generate_weapon(REMOVE_OLD, DO_WRITE_TXT=True):
    ...
    st = perf_counter()
    global faction_all_weapon, WEAPON_PARAMS
    faction_all_weapon, WEAPON_PARAMS = parse_all_weapon(
        mod_weapon_dir, all_weapon_path, mod_text_path)
    logger.info(f"耗时{perf_counter()-st:.3f}s")
...
def generate_carryitem(REMOVE_OLD, DO_WRITE_TXT=True):
    ...
    gval.set_value("REMOVE_OLD", REMOVE_OLD)
    gval.set_value("DO_WRITE_TXT", DO_WRITE_TXT)
    global faction_all_ci, CARRY_PARAMS
    faction_all_ci, CARRY_PARAMS = parse_all_carryitem(mod_items_dir,
                                                       all_carryitem_path,
                                                       mod_text_path)
